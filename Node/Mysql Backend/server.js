...
app.delete('/guestbookDelete/:id', (req, res) => {
  ...
  connection.query(
    ...
    function (err, results, fields){
      if(results.affectedRows === 0){
        return res.status(400).send({ error: `No row found with id: ${req.params.id}` });
      }
      res.send(`Row with id : ${req.params.id} succesfully deleted !`)
    }
  )
})
...
app.put("/guestbook/:id", (req, res) => {
  ...
  connection.query(
    "UPDATE guestbook SET pseudo = ?, message = ? WHERE id = ?",
    [data.pseudo, data.message, req.params.id],
    function(err, results, fields){
      if(err){
        return res.json({ success: false, message: err });
      }
